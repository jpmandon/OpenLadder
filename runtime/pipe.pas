  ...
  if i>0 then
     for j:=0 to i-1 do
	    ...
	    begin
         ...
         // is it a timer ?
         if checkVarList[j][1]='*' then
	    for n:=0 to timerlist.count-1 do		
		if checkVarList[j]='*'+timerlist[n].timerName then
			if timerList[n].timerEnabled then 
		           writeln('*'+timerList[n].timerName+'=enabled')
			else
		           writeln('*'+timerList[n].timerName+'=disabled');	    
         for n:=0 to bitList.count-1 do
